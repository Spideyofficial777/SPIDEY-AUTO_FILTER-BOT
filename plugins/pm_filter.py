            ...
            try:
                ...
                if btn:
                    btn.append(
                        [
                            InlineKeyboardButton(
                                "Unmute Me 🔕", callback_data=f"unmuteme#{int(user_id)}"
                            )
                        ]
                    )
        ...
        if manual == False:
            ...
            try:
                if settings["auto_ffilter"]:
                    ...
                    await auto_filter(
                        client, message.text, message, reply_msg, ai_search
                    )
            except KeyError:
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        0,
        [InlineKeyboardButton(text="sᴇʟᴇᴄᴛ ʏᴏᴜʀ ʏᴇᴀʀ", callback_data="ident")],
    )
    ...
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data=f"fy#homepage#{key}"
            )
        ]
    )
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not settings["button"]:
        ...
        remaining_seconds = "{:.2f}".format(time_difference.total_seconds())
        cap = await get_cap(
            settings, remaining_seconds, files, query, total_results, search
        )
        try:
            await query.message.edit_text(
                text=cap,
...
async def episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data=f"fe#homepage#{key}"
            )
        ]
    )
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        0,
        [InlineKeyboardButton(text="👇 𝖲𝖾𝗅𝖾𝖼𝗍 𝖸𝗈𝗎𝗋 𝖫𝖺𝗇𝗀𝗎𝖺𝗀𝖾𝗌 👇", callback_data="ident")],
    )
    req = query.from_user.id
    offset = 0
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ​↭", callback_data=f"fl#homepage#{key}"
            )
        ]
    )
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        0,
        [InlineKeyboardButton(text="👇 𝖲𝖾𝗅𝖾𝖼𝗍 Season 👇", callback_data="ident")],
    )
    req = query.from_user.id
    offset = 0
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ​↭", callback_data=f"next_{req}_{key}_{offset}"
            )
        ]
    )
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    files = [
        file for file in files if re.search(seas, file["file_name"], re.IGNORECASE)
    ]
    ...
    files1 = [
        file for file in files1 if re.search(seas1, file["file_name"], re.IGNORECASE)
    ]
    ...
    files2 = [
        file for file in files2 if re.search(seas2, file["file_name"], re.IGNORECASE)
    ]
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        0,
        [InlineKeyboardButton(text="⇊ ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ǫᴜᴀʟɪᴛʏ ⇊", callback_data="ident")],
    )
    req = query.from_user.id
    offset = 0
    btn.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if not free_trial_status:
            await db.give_free_trail(user_id)  # Keeping the function name as you wrote it
            new_text = (
            "<b>ᴄᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴꜱ🎉 ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ꜰʀᴇᴇ ᴛʀɪᴀʟ ꜰᴏʀ <u>5 ᴍɪɴᴜᴛᴇs</u> ꜰʀᴏᴍ ɴᴏᴡ ! "
            "ɴᴏᴡ ᴇxᴘᴇʀɪᴇɴᴄᴇ ᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ꜱᴇʀᴠɪᴄᴇ ꜰᴏʀ 5 ᴍɪɴᴜᴛᴇꜱ. "
            "ᴛᴏ ʙᴜʏ ᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ꜱᴇʀᴠɪᴄᴇ ᴄʟɪᴄᴋ ᴏɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ.</b>"
            )
            await query.message.edit_text(
                text=new_text,
                disable_web_page_preview=True,
        ...
        await client.edit_message_media(
            ...
            query.message.id, 
            InputMediaPhoto("https://graph.org/file/86d22de443c9cc9901a97-20eaf180564cd37df7.jpg")
        )
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('⚜️ ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ɢᴇᴛ ꜰʀᴇᴇ ᴛʀɪᴀʟ', callback_data="get_trail")
        ],[
...
#Deendayal403
        ...
        buttons = [[
            InlineKeyboardButton('☎️ ᴄᴏɴᴛᴀᴄᴛ ᴏᴡɴᴇʀ ᴛᴏ ᴋɴᴏᴡ ᴍᴏʀᴇ', user_id=int(7965267063))
        ],[
        ...
        buttons = [[
            InlineKeyboardButton('📲 ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ꜱᴄʀᴇᴇɴꜱʜᴏᴛ ʜᴇʀᴇ', user_id=int(7965267063))
        ],[
        ...
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto("https://graph.org/file/2adf45fb6d4a4bd1a68c9-cfd523fe6941af4404.jpg")
        )
        await query.message.edit_text(
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        await query.message.reply_photo(
            photo=PAYMENT_QR, caption=PAYMENT_TEXT, reply_markup=reply_markup
        )
        return
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings["is_shortlink"] and not await db.has_premium_access(
                query.from_user.id
            ):
                if clicked == typed:
                    temp.SHORT[clicked] = query.message.chat.id
                    await query.answer(
                        url=f"https://telegram.me/{temp.U_NAME}?start=short_{file_id}"
                    )
                    return
                else:
                    await query.answer(
            ...
            elif settings["is_shortlink"] and await db.has_premium_access(
                query.from_user.id
            ):
                if clicked == typed:
                    await query.answer(
        ...
        try:
            if settings["is_shortlink"] and not await db.has_premium_access(
                ...
                await query.answer(
                    url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles1_{key}"
                )
            elif settings["is_shortlink"] and await db.has_premium_access(
                query.from_user.id
            ):
                await query.answer(
                    url=f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}"
                )
                return
            else:
                await query.answer(
            ...
            await query.answer(
                url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles3_{key}"
            )
        except Exception as e:
            logger.exception(e)
            await query.answer(
                url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles4_{key}"
            )
        ...
        #      return
        files, n_offset, total = await get_search_results(
            query.message.chat.id, search, offset=int(offset), filter=True
        )
        await send_all(
            client,
            query.from_user.id,
            ...
            try:
                for file in files:
                    ...
                    if result.deleted_count:
                        logger.info(
                            f"File Found for your query {keyword}! Successfully deleted {file_name} from database."
                        )
                    deleted += 1
                    if deleted % 50 == 0:
                        await query.message.edit_text(
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    ...
                    InlineKeyboardButton(
                        "5 Mɪɴs" if settings["auto_delete"] else "✘ Oғғ",
                        callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{str(grp_id)}',
                    ),
                ],
                [
        ...
        btn2 = [
            ...
            [
                InlineKeyboardButton(
                    "Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/+KzbVzahVdqQ3MmM1"
                )
            ],
        ]
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(
                    chat_id=int(from_user),
                    text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ ʜᴀs ʙᴇᴇɴ ᴜᴘʟᴏᴀᴅᴇᴅ ʙʏ ᴏᴜʀ ᴍᴏᴅᴇʀᴀᴛᴏʀs. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.</b>",
                    reply_markup=InlineKeyboardMarkup(btn2),
                )
            except UserIsBlocked:
                await client.send_message(
                    chat_id=int(SUPPORT_CHAT_ID),
                    text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ ʜᴀs ʙᴇᴇɴ ᴜᴘʟᴏᴀᴅᴇᴅ ʙʏ ᴏᴜʀ ᴍᴏᴅᴇʀᴀᴛᴏʀs. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.\n\nNᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. Tᴏ sᴇɴᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ PM, Mᴜsᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</b>",
                    reply_markup=InlineKeyboardMarkup(btn2),
                )
        else:
            await query.answer(
                "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True
            )
        ...
        btn2 = [
            ...
            [
                InlineKeyboardButton(
                    "Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/+avUQP0oEXM44ZGM1"
                )
            ],
        ]
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(
                    chat_id=int(from_user),
                    text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ ᴏɴ ᴏᴜʀ ʙᴏᴛ's ᴅᴀᴛᴀʙᴀsᴇ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.</b>",
                    reply_markup=InlineKeyboardMarkup(btn2),
                )
            except UserIsBlocked:
                await client.send_message(
                    chat_id=int(SUPPORT_CHAT_ID),
                    text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ ᴏɴ ᴏᴜʀ ʙᴏᴛ's ᴅᴀᴛᴀʙᴀsᴇ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.\n\nNᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. Tᴏ sᴇɴᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ PM, Mᴜsᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</b>",
                    reply_markup=InlineKeyboardMarkup(btn2),
                )
        else:
            await query.answer(
        ...
        try:
            log_msg = await client.send_cached_media(
                chat_id=LOG_CHANNEL, file_id=file_id
            )
            ...
            button = [
                ...
                [
                    InlineKeyboardButton(
                        "• ᴡᴀᴛᴄʜ ɪɴ ᴡᴇʙ ᴀᴘᴘ •", web_app=WebAppInfo(url=stream)
                    )
                ],
            ]
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [
                ...
                [
                    InlineKeyboardButton(
                        "ᴘʀᴇᴍɪᴜᴍ ᴀɴᴅ ʀᴇғᴇʀʀᴀʟ", callback_data="reffff"
                    )
                ],
        ...
        await query.message.edit_text(
            text=script.SUBSCRIPTION_TXT.format(
                REFERAL_PREMEIUM_TIME, temp.U_NAME, query.from_user.id, REFERAL_COUNT
            ),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML,
        )
        ...
        btn = [[InlineKeyboardButton("⇋ ʙᴀᴄᴋ ⇋", callback_data="about")]]
        reply_markup = InlineKeyboardMarkup(btn)
        await query.message.edit_text(
    ...
    # Edit the message media to show a random image
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto(random.choice(PICS))
    ...
    # Edit the message text to show the source information
        await query.message.edit_text(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('• ᴜᴘᴅᴀᴛᴇs •', url="https://t.me/+9tdbATrOMLNlN2I1")
    ], [
            InlineKeyboardButton('• ʜᴀᴄᴋ •', url="https://t.me/+cMlrPqMjUwtmNTI1"),
            InlineKeyboardButton('• ᴍᴏᴠɪᴇғʟɪx •', url="https://t.me/SPIDEYOFFICIAL_777")
        ], [
            InlineKeyboardButton('• ᴀɴɪᴍᴇ ᴄʀᴜɪsᴇ •', url="https://t.me/+4nyaulfn0sliZTE1")
        ], [ 
            InlineKeyboardButton('• ʙᴀᴄᴋ •', callback_data='start')
        ]]
        ...
        stats = Spideydb.command("dbStats")
        used_dbSize = (stats["dataSize"] / (1024 * 1024)) + (
            stats["indexSize"] / (1024 * 1024)
        )
        free_dbSize = 512 - used_dbSize
        stats2 = sec_db.command("dbStats")
        used_dbSize2 = (stats2["dataSize"] / (1024 * 1024)) + (
            stats2["indexSize"] / (1024 * 1024)
        )
        free_dbSize2 = 512 - used_dbSize2
        stats3 = mydb.command("dbStats")
        used_dbSize3 = (stats3["dataSize"] / (1024 * 1024)) + (
            stats3["indexSize"] / (1024 * 1024)
        )
        ...
        btn = [
            ...
            [
                InlineKeyboardButton("ᴛᴀᴍɪʟ", callback_data="tamil_info"),
                InlineKeyboardButton("ᴇɴɢʟɪsʜ", callback_data="english_info"),
                InlineKeyboardButton("ʜɪɴᴅɪ", callback_data="hindi_info"),
            ],
            [
                InlineKeyboardButton("ᴍᴀʟᴀʏᴀʟᴀᴍ", callback_data="malayalam_info"),
                InlineKeyboardButton("ᴍᴀʀᴀᴛʜɪ", callback_data="marathi_info"),
                InlineKeyboardButton("ʙᴀɴɢʟᴀ", callback_data="bangladesh_info"),
            ],
            [
                InlineKeyboardButton("ᴛᴇʟᴜɢᴜ", callback_data="telugu_info"),
                InlineKeyboardButton("ᴋᴀɴɴᴀᴅᴀ", callback_data="kannada_info"),
                InlineKeyboardButton("ɢᴜɪᴀʀᴀᴛɪ", callback_data="gujarati_info"),
            ],
        ...
        btn = [
            [
                InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                InlineKeyboardButton(
                    "Cᴏɴᴛᴀᴄᴛ", url="telegram.me/hacker_x_official_777"
                ),
            ]
        ]
        await client.edit_message_media(
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    return await advantage_spell_chok(
                        client, name, msg, reply_msg, ai_search
                    )
                else:
...
async def advantage_spell_chok(client, name, msg, reply_msg, Spidey_search):
    mv_id = msg.id
    mv_rqst = name
    ...
    if AI_SPELL_CHECK == True and Spidey_search == True:
        Spidey_search_new = False
        Spidey_ai_msg = await reply_msg.edit_text(
            "<b><i>I Am Trying To Find Your Movie With Your Wrong Spelling.</i></b>"
        )
        movienamelist = []
        movienamelist += [movie.get("title") for movie in movies]
        for Spidey in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(Spidey[0]):
                await auto_filter(client, Spidey, msg, reply_msg, Spidey_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [
            [
